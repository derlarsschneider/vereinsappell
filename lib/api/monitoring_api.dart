...
class MonitoringApi {
  ...
  Future<Map<String, dynamic>> getPerfStats(String timeframe) async {
    final response = await _client.get(
      Uri.parse('${config.apiBaseUrl}/monitoring/perf?timeframe=$timeframe'),
      headers: headers(config),
    );
    if (response.statusCode == 200) {
      return json.decode(response.body);
    } else {
      throw Exception('Fehler beim Laden der Performance-Daten: ${response.statusCode}');
    }
  }
